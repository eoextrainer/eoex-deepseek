...
export default function SalesforceAdminDashboard() {
  ...
  const [brandVerifications, setBrandVerifications] = useState([
    {
      ...
      status: 'pending',
      documents: ['kbis', 'address_proof'],
      companyReg: 'RCS Paris 123456789',
    ...
    },
    {
      ...
      status: 'verified',
      documents: ['kbis', 'address_proof'],
      companyReg: 'RCS Lyon 987654321',
      taxNumber: '98765432109876',
      website: 'https://studio-polaris.fr',
      verifiedAt: '2026-01-26T16:45:00',
    },
    {
      id: 3,
      ...
      companyReg: 'RCS Marseille 555666777',
    },
  ]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setServices(servicesData.length ? servicesData : SERVICES);
  ...
  const handleRoleChange = async (userId, roleId) => {
    try {
      await userAPI.updateUser(userId, { role_id: Number(roleId) });
      await fetchDashboardData();
    } catch (error) {
  ...
  const handleAuthorizeExchange = async (integrationId) => {
    try {
      await integrationAPI.logExchange(integrationId, {
        status: 'authorized',
        message: 'Exchange authorized by system admin',
      });
      await fetchDashboardData();
    } catch (error) {
  ...
  const handleGeneratePrompt = async (featureRequestId, promptText) => {
    try {
      await featureRequestAPI.createAIPrompt(featureRequestId, { prompt_text: promptText });
      await fetchDashboardData();
    } catch (error) {
  ...
  const handleVerifyBrand = async (brandId) => {
    setBrandVerifications((prev) =>
      prev.map((brand) =>
        brand.id === brandId
          ? { ...brand, status: 'verified', verifiedAt: new Date().toISOString() }
          : brand
      )
    );
  };
  ...
  const handleRejectBrand = async (brandId, reason) => {
    setBrandVerifications((prev) =>
      prev.map((brand) =>
        brand.id === brandId
          ? { ...brand, status: 'rejected', rejectionReason: reason }
          : brand
      )
    );
  };
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 border-b" style={{ borderColor: 'color-mix(in srgb, var(--text-color) 20%, transparent)' }}>
          <div className="flex gap-8 overflow-x-auto">
            {['overview', 'performance', 'interactions', 'integrations', 'ai-analysis', 'brand-verification'].map((tab) => (
              <button
                key={tab}
              ...
              >
                ...
                {tab === 'brand-verification' && '🛡️ Vérification Marques'}
              </button>
            ))}
          </div>
        </div>
        ...
        {activeTab === 'overview' && (
          <>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                <div>
                  <h2 className="text-2xl font-bold" style={{ color: 'var(--text-color)' }}>
                    🛡️ Centre de Contrôle Plateforme
                  </h2>
                  <p className="text-sm mt-2" style={{ color: 'var(--text-color)', opacity: 0.7 }}>
                    Contrôle global, attribution des privilèges, orchestration des services et gouvernance des échanges.
                  </p>
                </div>
                <div className="flex flex-wrap gap-3">
                  ...
                  >
                    Ouvrir dashboard perf
                  </button>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                  { title: 'Contrôle des privilèges', value: `${users.length} utilisateurs`, hint: 'Gestion des rôles & statuts' },
                  { title: 'Services abonnés', value: `${subscriptions.length}`, hint: 'Gestion des services actifs' },
                  { title: 'Échanges API', value: `${integrations.length}`, hint: 'Microservices externes' },
                  { title: 'Prompts IA', value: `${aiPrompts.length}`, hint: 'Demandes transcrites' },
                ].map((card, idx) => (
                  <div
                    key={idx}
                    className="rounded-lg p-5 border"
                    style={{ borderColor: 'color-mix(in srgb, var(--primary-color) 25%, transparent)', backgroundColor: 'color-mix(in srgb, var(--secondary-color) 75%, transparent)' }}
                  >
                    <p className="text-sm uppercase" style={{ color: 'var(--text-color)', opacity: 0.7 }}>{card.title}</p>
                    <p className="text-2xl font-bold mt-2" style={{ color: 'var(--text-color)' }}>{card.value}</p>
                    <p className="text-xs mt-2" style={{ color: 'var(--text-color)', opacity: 0.6 }}>{card.hint}</p>
                  </div>
                ))}
              </div>
            </div>
                        ...
                        <td className="py-3 px-4 text-sm">
                          ...
                          >
                            {roles.length > 0 ? (
                              ...
                              <>
                                <option value={1}>system_admin</option>
                                <option value={2}>community_admin</option>
            ...
            {/* Service Subscriptions Detail */}
            <div className="rounded-lg shadow-sm p-8 mb-12" style={{ backgroundColor: 'var(--secondary-color)' }}>
              <h2 className="text-2xl font-bold mb-6" style={{ color: 'var(--text-color)' }}>
                📦 Services Souscrits par les Utilisateurs
              </h2>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="border-b" style={{ borderColor: 'color-mix(in srgb, var(--text-color) 20%, transparent)' }}>
                    <tr>
                      ...
                      <th className="text-left py-3 px-4 text-sm font-semibold" style={{ color: 'var(--text-color)', opacity: 0.8 }}>Service / Plan</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold" style={{ color: 'var(--text-color)', opacity: 0.8 }}>Statut</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold" style={{ color: 'var(--text-color)', opacity: 0.8 }}>Renouvellement</th>
                    </tr>
                  </thead>
                  <tbody>
                    {subscriptions.slice(0, 8).map((sub) => (
                      <tr key={sub.id} className="border-b" style={{ borderColor: 'color-mix(in srgb, var(--text-color) 10%, transparent)' }}>
                        <td className="py-3 px-4 text-sm" style={{ color: 'var(--text-color)' }}>
                          {users.find((u) => u.id === sub.user_id)?.full_name || `Utilisateur ${sub.user_id}`}
                        </td>
                        <td className="py-3 px-4 text-sm" style={{ color: 'var(--text-color)' }}>
                          {planMap[sub.plan_id]?.name || `Plan ${sub.plan_id}`}
                        </td>
                        <td className="py-3 px-4 text-sm" style={{ color: 'var(--primary-color)' }}>
                          {sub.status || 'active'}
                        </td>
                        <td className="py-3 px-4 text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>
                          {sub.renewal_date || '—'}
                        </td>
                      </tr>
                    ))}
          ...
          <div className="space-y-8">
            ...
            <div className="rounded-lg shadow-sm p-8" style={{ backgroundColor: 'var(--secondary-color)' }}>
              <h3 className="text-xl font-bold mb-4" style={{ color: 'var(--text-color)' }}>
                🔁 Échanges Microservices & Autorisations
              </h3>
              <p className="text-sm mb-6" style={{ color: 'var(--text-color)', opacity: 0.7 }}>
                Autorisez et facilitez les échanges de données entre la plateforme et les services externes.
              </p>
              <div className="space-y-4">
                {(integrations.length ? integrations : SERVICES).map((integration) => (
                  <div
                    key={integration.id}
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      <div>
                        <p className="font-semibold" style={{ color: 'var(--text-color)' }}>{integration.name || integration.service_name}</p>
                        <p className="text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>
                          {integration.endpoint || 'API externe'} • {integration.status || 'active'}
                        </p>
                      </div>
                      <button
                        className="px-4 py-2 rounded text-sm font-semibold"
                        style={{ backgroundColor: 'color-mix(in srgb, #22c55e 20%, transparent)', color: '#22c55e' }}
                        onClick={() => handleAuthorizeExchange(integration.id)}
                      >
                        Autoriser échange
                      </button>
                    </div>
                ...
                >
                  ...
                  <div className="flex gap-3">
                    <button
                      className="px-4 py-2 rounded text-sm font-semibold"
                      style={{ backgroundColor: 'color-mix(in srgb, #22c55e 20%, transparent)', color: '#22c55e' }}
                      onClick={() => handleGeneratePrompt(prompt.featureRequestId, prompt.prompt)}
                    >
                      Générer Prompt IA
                    </button>
                    <button
          ...
          <div>
            <div className="mb-8">
              <h2 className="text-2xl font-bold mb-2" style={{ color: 'var(--text-color)' }}>
                🛡️ Gestion de la Vérification des Marques
              </h2>
              <p className="text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>
                Examinez et approuvez les demandes de vérification des comptes marque. La vérification protège les talents contre les faux profils.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
              <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--secondary-color)' }}>
                <p className="text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>Total demandes</p>
                <p className="text-3xl font-bold mt-2" style={{ color: 'var(--primary-color)' }}>
                  {brandVerifications.length}
                </p>
              </div>
              <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--secondary-color)' }}>
                <p className="text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>En attente</p>
                <p className="text-3xl font-bold mt-2" style={{ color: '#f59e0b' }}>
                  {brandVerifications.filter((b) => b.status === 'pending').length}
                </p>
              </div>
              <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--secondary-color)' }}>
                <p className="text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>Vérifiées</p>
                <p className="text-3xl font-bold mt-2" style={{ color: '#22c55e' }}>
                  {brandVerifications.filter((b) => b.status === 'verified').length}
                </p>
              </div>
              <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--secondary-color)' }}>
                <p className="text-sm" style={{ color: 'var(--text-color)', opacity: 0.7 }}>Rejetées</p>
                <p className="text-3xl font-bold mt-2" style={{ color: '#ef4444' }}>
                  {brandVerifications.filter((b) => b.status === 'rejected').length}
                </p>
              </div>
            </div>
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
                      <div className="flex-1">
                        ...
                        <p className="text-xs mt-2" style={{ color: 'var(--text-color)', opacity: 0.6 }}>
                          Soumis le {new Date(brand.submittedAt).toLocaleString('fr-FR')}
                        </p>
                      </div>
                      <div className="flex flex-col gap-2">
                        <span
                          className="px-4 py-2 rounded text-xs font-semibold text-white text-center"
                          style={{
                            backgroundColor:
                              brand.status === 'pending'
                                ? '#f59e0b'
                        ...
                        >
                          {brand.status === 'pending' && '⏳ En attente'}
                          {brand.status === 'verified' && '✅ Vérifié'}
                          {brand.status === 'rejected' && '❌ Rejeté'}
                        </span>
                        {brand.verifiedAt && (
                          <p className="text-xs" style={{ color: '#22c55e' }}>
                            Approuvé le {new Date(brand.verifiedAt).toLocaleString('fr-FR')}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Company Information */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 rounded" style={{ backgroundColor: 'color-mix(in srgb, var(--primary-color) 5%, transparent)' }}>
                      <div>
                        <p className="text-xs" style={{ color: 'var(--text-color)', opacity: 0.7 }}>RCS/Immatriculation</p>
                        <p className="font-semibold" style={{ color: 'var(--text-color)' }}>{brand.companyReg}</p>
                      </div>
                      {brand.taxNumber && (
                        <div>
                          <p className="text-xs" style={{ color: 'var(--text-color)', opacity: 0.7 }}>SIRET/SIREN</p>
                          <p className="font-semibold" style={{ color: 'var(--text-color)' }}>{brand.taxNumber}</p>
                        </div>
                      )}
                      {brand.website && (
                        <div>
                          <p className="text-xs" style={{ color: 'var(--text-color)', opacity: 0.7 }}>Site officiel</p>
                          <a href={brand.website} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--primary-color)' }} className="font-semibold underline">
                            Visiter
                          </a>
                        </div>
                      )}
                    ...
                    <div className="mb-4">
                      ...
                      <div className="flex gap-2 flex-wrap">
                        {brand.documents.map((doc) => (
                          ...
                          >
                            {doc === 'kbis' && '✅ Extrait Kbis'}
                            {doc === 'address_proof' && '✅ Justificatif d\'adresse'}
                          </span>
                        ))}
                      </div>
                    </div>
                    ...
                    {brand.rejectionReason && (
                      <div className="mb-4 p-3 rounded" style={{ backgroundColor: 'color-mix(in srgb, #ef4444 10%, transparent)' }}>
                        <p className="text-sm font-semibold mb-1" style={{ color: '#ef4444' }}>
                          Motif du rejet:
                        </p>
                        <p style={{ color: 'var(--text-color)' }}>{brand.rejectionReason}</p>
                      </div>
                    )}
                      ...
                      <div className="flex gap-3">
                        ...
                        <button
                          onClick={() => {
                            ...
                            if (reason) handleRejectBrand(brand.id, reason);
                          }}
                          className="flex-1 px-4 py-2 rounded font-semibold text-white"
